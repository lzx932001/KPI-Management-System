        ...
        <fieldset>
            ...
            <ul> 
                <?php 
                    ...
                    if(isset($_GET['fname'])){  //check if the POST variable receive an input
                        if(mysqli_num_rows($result)>0){
                            $arrName = array();
                            while($row = mysqli_fetch_assoc($result)){      //loop through the database, which contains all the Staff information
                                if(str_contains(strtolower($row['name']), strtolower($_GET['fname']))){    //if it matches with input, show the link to the staff info page
                                    echo "<li>";
                                    echo "<a href='UpdateStaffKPI.php?name=$row[name]'>$row[name]</a>";
                                    echo "</li>";
